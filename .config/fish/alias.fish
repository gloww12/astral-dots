# sl
alias paur="sl"
alias tetris="sl"
alias pcaman='sl'
alias :w='sl'
alias claer='sl'
alias sudo='sl'
alias bash='sl'
alias man='sl'
alias paru='sl'

alias 'sl'='/home/glow/.config/fish/randsl.sh'

# BULLSHIT aliases
alias HELP='/usr/bin/env bash'
alias please='/usr/bin/env sudo'
alias mods='/usr/bin/env sudo'
alias fucking='/usr/bin/env sudo'
alias rtfm='/usr/bin/env man'
alias g='/usr/bin/env git'

# real aliases
alias orpheus='cd /home/glow/OrpheusDL && python orpheus.py'
alias v="vim"
alias sv="EDITOR=vim sudoedit"
alias nv="nvim"
alias snv="sudoedit"
alias :q="exit"
alias wanger="ranger"
# disabled because i use multiple venvs now
# alias python="/home/glow/.pyvenv/bin/python"
# alias pip="/home/glow/.pyvenv/bin/pip"
alias py3.13='source /home/glow/.pyvenv3.13/bin/activate.fish'
alias py3.13t='source /home/glow/.pyvenv3.13t/bin/activate.fish'
alias py3.12='source /home/glow/.pyvenv3.12/bin/activate.fish'
alias j17='please archlinux-java set java-17-openjdk'
alias j21='please archlinux-java set java-21-openjdk'
alias j25='please archlinux-java set java-25-openjdk'
alias py='source /home/glow/.pyvenv/bin/activate.fish'
alias fuck='/usr/bin/env paru -Rnsc'
alias ber='bundle exec rubocop'
alias be='bundle exec'
alias br='bin/rails'
alias chat-what='/usr/bin/env paru -Ss'
alias search='/usr/bin/env paru -Ss'
alias p='/usr/bin/env paru'
alias poo='/usr/bin/env paru'
alias pee='/usr/bin/env paru'
alias md='mkdir -p'
alias nuke='pkill -KILL'
alias s='source /home/glow/.config/fish/config.fish'
alias uvpn='please openfortivpn remoteaccess.shef.ac.uk -u ach22jc'
alias rosbox='distrobox enter rosbox'
alias rails-server='thrust bin/rails s'
alias updf='cd /home/glow/Documents/obsidian/uni/attachments && nohup zathura $(fzf)'
alias condascript='source /home/glow/.config/fish/condascript.fish'
alias cpip='/home/glow/.conda/envs/sa_cell_tools/bin/pip'
alias bark='ruff format /home/glow/Documents/job/staph-code/cell_tool/'
alias main='git switch main'
alias branch='git checkout -b'
alias cocofetch='fastfetch -l /home/glow/bullshit/coconut.jpg --logo-width 25'
alias neofetch='fastfetch -l /home/glow/bullshit/coconut.jpg --logo-width 25'
alias esp='source /opt/esp-idf/export.fish'
alias t='todo.sh'
alias img='kitten icat'
alias imdb='cat ~/imdb.md'
alias lwc='texcount -inc -sum'
alias msg='noctalia msg'
